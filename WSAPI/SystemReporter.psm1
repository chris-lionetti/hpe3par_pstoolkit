...
Param(	[Switch]	$VersusTime,
		...
		[String]	$Compareby,
		[int]		$NoOfRecords,	
		[ValidateSet('hitIORead','hitIOWrite','missIORead','missIOWrite','accessIORead','accessIOWrite','hitPctRead','hitPctWrite','totalAccessIO',
		'lockBulkIO','pageStatisticDelayAckPagesNL_7','pageStatisticDelayAckPagesFC','pageStatisticDelayAckPagesSSD','pageStatisticPageStatesFree',
		'pageStatisticPageStatesClean','pageStatisticPageStatesWriteOnce','pageStatisticPageStatesWriteMultiple','pageStatisticPageStatesWriteScheduled',
		'pageStatisticPageStatesWriteing','pageStatisticPageStatesDcowpend','pageStatisticDirtyPagesNL','pageStatisticDirtyPagesFC',
		'pageStatisticDirtyPagesSSD','pageStatisticMaxDirtyPagesNL_7','pageStatisticMaxDirtyPagesFC','pageStatisticMaxDirtyPagesSSD')]
...
{	$Result = $null
	...
	{ 	$cmpVal = $Compareby.ToLower()
		...
		{	write-error "Compareby should be either top or bottom"
			return 
		}
		if($NoOfRecords)	{	$uri = $uri+$NoOfRecords+","	}
		else	{	write-Error "NoOfRecords is mandatory with Compareby. "
					return 
				}
		if($ComparebyField)	{	$uri = $uri+$ComparebyField	}
		else	{	write-error "ComparebyField is mandatory with Compareby.please see the parameter help for this." 
					return 
				}		
	}
	if($GETime)
...
.PARAMETER GETime
	Gerater thane time For At Time query expressions, you can use the sampleTime parameter
.PARAMETER LETime
	Less than time For At Time query expressions, you can use the sampleTime parameter
.EXAMPLE
...
{	$Result = $null
	...
	{ 	$cmpVal = $Compareby.ToLower()
		...
		else											{	Write-Error "Compareby should be either top or bottom"
															return 
														}
		if($NoOfRecords)	{	$uri = $uri+$NoOfRecords+","	}
		else				{	write-Error "NoOfRecords is mandatory with Compareby. "
...
{	$Result = $null
	...
	if($VersusTime)	{	$Action = "vstime"	}	
	elseif($AtTime)	{	$Action = "attime"	}	
	else			{	Write-Error "Please Select at-list any one from Versus Time or At Time for statistics report."
						Return 
					}
	if($Frequency_Hires)		{	$Frequency = "hires"	}	
	elseif($Frequency_Hourly)	{	$Frequency = "hourly"	} 
...
{	$Result = $null
	...
	if($NodeId) { 	if($AtTime) 	{ 	Write-Error "We cannot pass node values in At Time report."
										return  
									} 
					$uri = $uri+";node:$NodeId"
				}
...
{	$Result = $null
	...
	if($VvName) { 	if($AtTime) 	{ 	write-error "We cannot pass VvName in At Time report."
										return 
									}
					$uri = $uri+";name:$VvName"
				}
	if($VvSetName) 	{ 	if($AtTime) { 	write-error "We cannot pass VvSetName in At Time report." 
										return
									} 
						$uri = $uri+";name:set:$VvSetName"
					}
	if($UserCPG) 	{ 	if($AtTime) 	{ 	write-error "We cannot pass UserCPG in At Time report." 
											return
										} 
						$uri = $uri+";userCPG:$UserCPG"
					}
	if($SnapCPG) 	{ 	if($AtTime) 	{ 	write-error "We cannot pass SnapCPG in At Time report." 
											return
										} 
						$uri = $uri+";snapCPG:$SnapCPG"
					}
	if($ProvType) 	{ 	if($AtTime) 	{ 	write-error "We cannot pass ProvType in At Time report." 
										} 
						$uri = $uri+";provType:$ProvType"
					}		
	if($Groupby) 	{  	$uri = $uri+";groupby:$Groupby"	}
	if($Summary) 	{ 	$uri = $uri+";summary:$Summary"	}
    if($Compareby)
