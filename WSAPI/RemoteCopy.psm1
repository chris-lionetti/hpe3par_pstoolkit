...
Param(	[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		...
		[String]	$TargetName,
		[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		[ValidateSet('SYNC','PPERIODIC','ASYNC')]
		[String]	$Mode,
		[String]	$UserCPG,
		[String]	$SnapCPG,
		[String]	$LocalUserCPG,
...
{	$body = @{}	
	...
	if($status -eq 201)
	{	write-host "`n SUCCESS: Remote Copy group : $RcgName created successfully.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While Creating a Remote Copy group : $RcgName `n`n" 
...
{  	$body = @{}
	...
	{	write-host "`n SUCCESS: Successfully Start a Remote Copy group.`n" -foreground green
		return $Result		
	}
	else
	{	write-error "`n FAILURE : While Starting a Remote Copy group.`n`n"
		return $Result.StatusDescription
	}
}
}
...
{  	$body = @{}
    ...
    $Result = Invoke-WSAPI -uri $uri -type 'PUT' -body $body
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Successfully Stop a Remote Copy group.`n" -foreground green
		return $Result		
	}
	else
	{	write-error "`n FAILURE : While Stopping a Remote Copy group.`n"
		return $Result.StatusDescription
	}
}
}
...
{  	$body = @{}
    ...
    $Result = Invoke-WSAPI -uri $uri -type 'PUT' -body $body
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Successfully Synchronize a Remote Copy group.`n" -foreground green
		return $Result		
	}
	else
	{	write-error "`n FAILURE : While Synchronizing a Remote Copy group.`n"
		return $Result.StatusDescription
	}
}
}
...
{   $uri = '/remotecopygroups/'+ $GroupName
	...
	$status = $Result.StatusCode
	if($status -eq 202)
	{	write-host "`n SUCCESS: Remove a Remote Copy group:$GroupName successfully remove.`n" -foreground green
		return
	}
	else
	{	write-error "`n FAILURE : While Removing a Remote Copy group : $GroupName `n"
		return $Result.StatusDescription
	}    
}
}
...
Param(	[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		...
		[String]	$LocalSnapCPG,	  
		[String]	$TargetName,
		[String]		$RemoteUserCPG,
		[String]	$RemoteSnapCPG,
		[int]		$SyncPeriod,
		[int]		$RmSyncPeriod,
		...
		[int]		$AutoFailover,
		[int]		$PathManagement,
		[int]		$MultiTargetPeerPersistence
)
Begin 
{	Test-WSAPIConnection
}
...
{  	$body = @{}	
	...
	if($Mode -eq "PERIODIC"){	$MOD=@{}
								$MOD["mode"] = 3
								$TargetsBody += $MOD		
							}
	if($Mode -eq "ASYNC")	{	$MOD=@{}
								$MOD["mode"] = 4
								$TargetsBody += $MOD		
							}
	...
	$Result = Invoke-WSAPI -uri $uri -type 'PUT' -body $body
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Successfully Update Remote Copy group.`n" -foreground green
		return (Get-System_WSAPI)
	}
	else
	{	write-error "`n FAILURE : While Updating Remote Copy group.`n"
		return $Result.StatusDescription
	}
}
}
...
Param(	[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		...
		[String]	$TargetName,
		[int]		$SnapFrequency,
		[Boolean]	$RmSnapFrequency,
		[int]		$SyncPeriod,
		[Boolean]	$RmSyncPeriod,
...
{  	$body = @{}	
    ...
    $Result = Invoke-WSAPI -uri $uri -type 'PUT' -body $body
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Successfully Update Remote Copy group target.`n" -foreground green
		return (Get-System_WSAPI)
	}
	else
	{	write-error "`n FAILURE : While Updating Remote Copy group target.`n"
		return $Result.StatusDescription
	}
}
}
...
Param(	[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		...
		[Switch]	$SkipStart,
		[Switch]	$SkipSync,
		[Switch]	$DiscardNewData,
		[Switch]	$SkipPromote,
		[Switch]	$NoSnapshot,
		[Switch]	$StopGroups,
		[Switch]	$LocalGroupsDirection
)
Begin 
{	Test-WSAPIConnection
}
...
{	Write-DebugLog "Running: Creation of the body hash" $Debug
	...
	$status = $Result.StatusCode
	if($status -eq 200)
	{	write-host "`n SUCCESS: Remote Copy group : $GroupName successfully Recover.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While Recovering a Remote Copy group : $GroupName `n"
		return $Result.StatusDescription
	}
}
}
...
Param(	[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		...
		[String]	$VolumeName,
		[String]	$SnapshotName,
		[boolean]	$VolumeAutoCreation,
		[boolean]	$SkipInitialSync,
		[boolean]	$DifferentSecondaryWWN,
		[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		[String]	$TargetName,
		[Parameter(Mandatory=$true, ValueFromPipeline=$true)]
		[String]	$SecVolumeName	
	)
Begin 
{	Test-WSAPIConnection
}
...
{	$body = @{}
	...
	$status = $Result.StatusCode
	if($status -eq 200)
	{	write-host "`n SUCCESS: Volume into a Remote Copy group : $VolumeName successfully Admitted.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While Admitting a volume into a Remote Copy group : $VolumeName `n"
		return $Result.StatusDescription
	}
}
}
...
{	$body = @{}
	...
	$status = $Result.StatusCode
	if($status -eq 200)
	{	write-host "`n SUCCESS: Volume from a Remote Copy group : $VolumeName successfully Remove.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While Dismissing a volume from a Remote Copy group : $VolumeName `n"
		return $Result.StatusDescription
	}
}
}
...
{	$body = @{}
	...
	else			{	return "Please select at-list any one from IP or FC Type." }
	...
	$status = $Result.StatusCode
	if($status -eq 201)
	{	write-host "`n SUCCESS: Remote Copy Target : $TargetName created successfully.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While creating a Remote Copy target : $TargetName `n"
		return $Result.StatusDescription
	}
}
}
...
{ 	$body = @{}
    ...
    $Result = Invoke-WSAPI -uri $uri -type 'PUT' -body $body
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Successfully Update Remote Copy Target / Target Name : $TargetName.`n" -foreground green
		return
	}
	else
	{	write-error "`n FAILURE : While Updating Remote Copy Target / Target Name : $TargetName.`n"
		return $Result.StatusDescription
	}
}
}
...
{	Write-DebugLog "Running: Creation of the body hash" $Debug
	...
	$status = $Result.StatusCode
	if($status -eq 200)
	{	write-host "`n SUCCESS: Successfully Admitted a target into a Remote Copy group : TargetName = $TargetName / GroupName = $GroupName.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While admitting a target into a Remote Copy group : TargetName = $TargetName / GroupName = $GroupName `n"
		return $Result.StatusDescription
	}
}
}
...
{   $Result = $null
	...
	$status = $Result.StatusCode
	if($status -eq 201)
	{	write-host "`n SUCCESS: Successfully Remove a target from a Remote Copy group : TargetName = $TargetName / GroupName = $GroupName.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While removing  a target from a Remote Copy group : TargetName = $TargetName / GroupName = $GroupName `n"
		return $Result.StatusDescription
	}
}
...
{	$body = @{}
	...
	$status = $Result.StatusCode
	if($status -eq 201)
	{	write-host "`n SUCCESS: Successfully Create coordinated snapshots across all Remote Copy group volumes.`n" -foreground green
		return $Result
	}
	else
	{	write-error "`n FAILURE : While Creating coordinated snapshots across all Remote Copy group volumes.`n"
		return $Result.StatusDescription
	}
}
}
...
{	$Result = $null
	...
	{	$dataPS = $Result.content | ConvertFrom-Json
		write-host "`n SUCCESS: Command Get-RCopyInfo_WSAPI Successfully Executed`n" -foreground green
		return $dataPS
	}
	else
	{	write-error "`n FAILURE : While Executing Get-RCopyInfo_WSAPI.`n"
		return $Result.StatusDescription
	}
}	
}
...
{	$Result = $null
	...
	{	$Result = Invoke-WSAPI -uri '/remotecopytargets' -type 'GET'
		if($Result.StatusCode -eq 200)	{	$dataPS = ($Result.content | ConvertFrom-Json).members	}		
	}
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Command Get-RCopyTarget_WSAPI Successfully Executed.`n" -foreground green
		return $dataPS
	}
	else
	{	write-error "`n FAILURE : While Executing Get-RCopyTarget_WSAPI.`n"
		return $Result.StatusDescription
	}
}	
}
...
{	$Result = $null
	...
	{	$Result = Invoke-WSAPI -uri '/remotecopygroups' -type 'GET'
		if($Result.StatusCode -eq 200)	{	$dataPS = ($Result.content | ConvertFrom-Json).members	}		
	}
	if($Result.StatusCode -eq 200)
	{	if($dataPS.Count -gt 0)
		{	write-host "`n SUCCESS: Command Get-RCopyGroup_WSAPI Successfully Executed.`n" -foreground green
			return $dataPS
		}
		else
		{	write-error "`n FAILURE : While executing Get-RCopyGroup_WSAPI. Expected result not found with given filter option .`n"
			return 
		}
	}
	else
	{	write-error "`n FAILURE : While Executing Get-RCopyGroup_WSAPI.`n"
		return $Result.StatusDescription
	}
}	
}
...
{	$Result = $null
	...
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Command Get-RCopyGroupTarget_WSAPI Successfully Executed.`n" -foreground green
		return $dataPS		
	}
	else
	{	write-error "`n FAILURE : While Executing Get-RCopyGroupTarget_WSAPI.`n"
		return $Result.StatusDescription
	}
}	
}
...
{	$Result = $null
	...
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Command Get-RCopyGroupVv_WSAPI Successfully Executed.`n" -foreground green
		return $dataPS		
	}
	else
	{	write-error "`n FAILURE : While Executing Get-RCopyGroupVv_WSAPI.`n"
		return $Result.StatusDescription
	}
}	
}
...
{	$Result = $null
	...
	{	$Result = Invoke-WSAPI -uri '/remotecopylinks' -type 'GET'
		if($Result.StatusCode -eq 200)	{	$dataPS = ($Result.content | ConvertFrom-Json).members }	
	}	  
	if($Result.StatusCode -eq 200)
	{	write-host "`n SUCCESS: Command Get-RCopyLink_WSAPI Successfully Executed.`n" -foreground green
		return $dataPS		
	}
	else
	{	write-error "`n FAILURE : While Executing Get-RCopyLink_WSAPI.`n"
		return $Result.StatusDescription
	}
}	
}
