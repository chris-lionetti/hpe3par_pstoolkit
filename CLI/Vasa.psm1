...
.PARAMETER Listcols
	...
	By default with mandatory option -sc, (if none of the information selection options below are specified) the 
	following columns are shown: VM_Name GuestOS VM_State Num_vv Physical Logical
.PARAMETER Detailed
...
.PARAMETER VVAssociatedWithVM
	...
	The following columns are shown:	VM_Name VV_ID VVol_Name VVol_Type Prov Physical Logical

	The columns displayed can change when used with other options. See the -binding option above.
...
param(	[String]	$container_name,
		...
		[String]	$ShowCols,
		[switch]	$Detailed,
		[switch]	$StorageProfiles,
		[switch]	$Summary,
		[switch]	$Binding,
		[switch]	$VVAssociatedWithVM,
		[switch]	$RemoteCopy,
		[switch]	$AutoDismissed,
		[String]	$VM_name
	)	
Begin
{	Test-CLIConnectionB
...
<#
.SYNOPSIS
    The Get-vVolSc command displays VVol storage containers, used to contain VMware Volumes for Virtual Machines (VVols).
.DESCRIPTION
    The Get-vVolSc command displays VVol storage containers, used to contain VMware Volumes for Virtual Machines (VVols).
.EXAMPLE
	Get-vVolSc 
...
.PARAMETER Detailed
	Displays detailed information about the storage containers, including any VVols that have been auto-dismissed by remote copy DR operations.
.PARAMETER SC_name  
	Storage Container
#>
[CmdletBinding()]
...
.EXAMPLE
	...
	(Note: set: already include in code please dont add with vvset)
.PARAMETER Create
...
.PARAMETER Remove
	If the specified VV set is a VVol storage container, this option will remove the VV set storage container and remove all of the 
	associated volumes. The user will be asked to confirm that the associated volumes in this storage container should be removed.
.PARAMETER Keep
	Used only with the -create option. If specified, allows a VV set with existing volumes to be marked as a VVol storage container.  
	However, this option should only be used if the existing volumes in the VV set are VVols.
.PARAMETER vvset
	The Virtual Volume set (VV set) name, which is used, or to be used, as a VVol storage container.
#>
[CmdletBinding()]
param(	[Parameter(Mandatory=$True)]							[String]	$vvset,
		...
		[Parameter(Mandatory=$True,ParameterSetName='Keep')]	[switch]	$Keep
	)	
Begin	
{	Test-CLIConnectionB
}
