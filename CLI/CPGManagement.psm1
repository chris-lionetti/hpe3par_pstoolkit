...
.EXAMPLE
	Get-CPG -Detailed -cpgName "MyCPG"  Displays detailed information about the CPGs.
.EXAMPLE
	Get-CPG -RawSpace -cpgName "MyCPG" Specifies that raw space used by the CPGs is displayed.
.EXAMPLE
	Get-CPG -AlertTime -cpgName "MyCPG"  Show times when alerts were posted (when applicable).
.EXAMPLE
	Get-CPG -Domain_Name XYZ -cpgName "MyCPG" Show times with domain name depict.
.PARAMETER cpgName 
...
.PARAMETER Detailed
	...
	Id Name Warn% VVs TPVVs TDVVs UsageUsr UsageSnp Base SnpUsed Free Total LDUsr LDSnp RC_UsageUsr RC_UsageSnp DDSType DDSSize
.PARAMETER RawSpace
...
.PARAMETER Alert
	Indicates whether alerts are posted. The following columns are shown: Id Name Warn% UsrTotal DataWarn DataLimit 
	DataAlertW% DataAlertW DataAlertL DataAlertF
.PARAMETER Alerttime
...
{	$GetCPGCmd = "showcpg "
	...
	{	$objType = "cpg"
		...
		if ( -not ( Test-CLIObject -objectType $objType -objectName $cpgName -objectMsg $objMsg -SANConnection $SANConnection)) 
			{	Throw "FAILURE : No cpg $cpgName found"
			}
		$GetCPGCmd += "  $cpgName"
	}	
	...
	{	$3parosver = Get-Version -S -SANConnection  $SANConnection
		...
		{	if($Alert -Or $AlertTime -Or $SAG -Or $SDG)
			{	$Cnt
				...
				if ($CPGName)
				{ 	Import-Csv $tempFile | where-object  {$_.Name -like $CPGName} 
				}
				else
				{	Import-Csv $tempFile
				}			
...
.PARAMETER SDGW
	...
	GB (using g or G) or TB (using t or T). A size of 0 (default) means no warning limit is enforced. To set the warning for any used space,
	set the limit to 1.
.PARAMETER saLD_name
...
.PARAMETER SS
	...
	If SSD drives are used, the step size defaults to 32 KB for RAID-0 and RAID-1, and 64 KB for RAID-5. For RAID-6, the default is a function of the
	set size.
.PARAMETER HA
	Specifies that the layout must support the failure of one port pair, one cage, or one drive magazine (mag). This option has no meaning
...
{	if ($cpgName)
	{	$objType = "cpg"
		...
		{	$RemoveCPGCmd +=" -f "
			...
			{	throw "FAILURE: While removing cpg $cpgName `n $Result3"
			}
			else
			{
				if ($Result3 -match "Removing CPG")
				{	Write-Host "Success : Removed cpg $cpgName" -ForegroundColor Green 
					return 
				}
				else
				{
					Throw "FAILURE: While removing cpg $cpgName $Result3"
				}
			}			
		}		
	}
...
.PARAMETER Ss
	...
	device type used. If no value is entered and FC or NL drives are used, the step size defaults to 256 KiB for RAID-1.
	If SSD drives are used, the step size defaults to 32 KiB for RAID-1. For RAID-6, the default is a function of the set size.
.PARAMETER Ha
