...
.EXAMPLE
	Get-Maint -All 
.PARAMETER All
	Display all maintenance window records, including active and expired ones. If this option is not 
	specified, only active window records will be displayed.
.PARAMETER Sortcol
...
.PARAMETER Comment
	Specifies any comment or additional information for the maintenance window record. The comment can be up to 
	255 characters long. Unprintable characters are not allowed.
.PARAMETER Duration
	Sets the duration of the maintenance window record. May be specified in minutes (e.g. 20m) or 
	hours (e.g. 6h). Value is not to exceed 24 hours. The default is 4 hours.
.PARAMETER MaintType
	Specify the maintenance type. Maintenance type can be Other, Node, Restart, Disk, Cage, 
	Cabling, Upgrade, DiskFirmware, or CageFirmware.
#>
[CmdletBinding()]
param(	[String]	$Comment,
		[String]	$Duration,
		[Parameter(Mandatory=$true)]
		[ValidateSet('Other', 'Node', 'Restart', 'Disk', 'Cage', 'Cabling', 'Upgrade', 'DiskFirmware', 'CageFirmware', 'all')]
		[String]	$MaintType
)
Begin
{	Test-CLIConnectionB
...
.EXAMPLE
.PARAMETER Comment
	Specifies any comment or additional information for the maintenance window record. Unprintable characters are not allowed.
.PARAMETER Duration
	Extends the duration of the maintenance window record by the specified time. May be specified in minutes (e.g. 20m) or 
	hours (e.g. 6h). If unspecified, the window duration is unchanged. This option cannot be specified with the -end option.
.PARAMETER End
	Ends the window record for the specified maintenance type. If the maintenance window record has been created more than 
	once with "createmaint", this option reduces its reference count by 1 without ending the window record. 
	This option cannot be specified with the Duration Option.
.PARAMETER MaintType
...
param(	[ValidateLength(1,255)]						[String]	$Comment,
		[Parameter(ParameterSetName='ByTime')]		[String]	$Duration,
		[Parameter(ParameterSetName='ByEndTime')]	[switch]	$End,
		[ValidateSet('Other', 'Node', 'Restart', 'Disk', 'Cage', 'Cabling', 'Upgrade', 'DiskFirmware', 'CageFirmware', 'all')]
													[String]	$MaintType
)
Begin
