...
.PARAMETER Pat
	Specifies that the <VV_name> is treated as a glob-style pattern and that all remote copy volumes matching the specified 
	pattern are admitted to the remote copy group. When this option is used the <sec_VV_name> and <snapname> (if specified) 
	are also treated as patterns. It is required that the secondary volume names and snapshot names can be derived from the 
	local volume name by adding a prefix, suffix or both. <snapname> and <sec_VV_name> should take the form prefix@vvname@suffix, 
	where @vvname@ resolves to the name of each volume that matches the <VV_name> pattern.
.PARAMETER CreateVV
	Specifies that the secondary volumes should be created automatically. This specifier cannot be used when starting 
	snapshots (<VV_name>:<snapname>) are specified.
.PARAMETER NoWWN
	When used with -createvv, it ensures a different WWN is	used on the secondary volume. 
	Without this option -createvv will use the same WWN for both primary and secondary volumes.
.PARAMETER NoSync
	Specifies that the volume should skip the initial sync. This is for the admission of volumes that have been pre-synced 
	with the target volume. This specifier cannot be used when starting snapshots (<VV_name>:<snapname>) are specified.
.PARAMETER SourceVolumeName
	Specifies the name of the existing virtual volume to be admitted to an existing remote copy volume group that was 
	created with the creatercopygroup command.
.PARAMETER Snapname
...
.PARAMETER TargetVolumeName
	The target name associated with this group, as set with the creatercopygroup command. The target is created with the
	creatercopytarget command. <sec_VV_name> specifies the name of the secondary volume on the target system.  One <target_name>:<sec_VV_name>
...
param(	[Parameter(Mandatory)]								[String]	$TargetName,
		[Parameter(ParameterSetName='NSPIP',Mandatory)]		[String]	$N_S_P_IP,
		[Parameter(ParameterSetName='NSPWWN',Mandatory)]	[String]	$N_S_P_WWN
	)
Begin	
{	Test-CLIConnectionB
}	
...
#>
[CmdletBinding()]
param(	[Parameter(ParameterSetName='RCIP', Mandatory)]
		[Switch]	$RCIP,
	
		[Parameter(ParameterSetName='RCFC', Mandatory)]
		[Switch]	$RCFC,
...
{ 	$cmd= "dismissrcopylink "
	if($RCFC)
		{	$cmd+=" $Target_name $NSP_IP_address "		
		}
	if($RCIP)
		{	$cmd+=" $Target_name $NSP_WWN "		
		}	
	$Result = Invoke-CLICommand -Connection $SANConnection -cmds  $cmd
	return 	$Result	
}	
...
.PARAMETER Pat
	Specifies that specified patterns are treated as glob-style patterns and all remote copy volumes matching the specified 
	pattern will be dismissed from the remote copy group. This option must be used if the <pattern> specifier is used.
.PARAMETER KeepSnap
	Specifies that the local volume's resync snapshot should be retained. The retained snapshot will reflect the state of 
	the secondary volume and might be used as the starting snapshot if the volume is readmitted	to a remote copy group. 
	The snapshot name will begin with "sv.rcpy"
.PARAMETER RemoveVV
...
#>
[CmdletBinding()]
param(	[ValidateRange(1,2147483647)]	[int]		$Interval,
		
										[switch]	$HeartBeat,
		
		[ValidateSet('k','m','g')]		[String]	$Unit = 'k',
		
		[Parameter(Mandatory)]
		[ValidateRange(1,2147483647)]	[int]		$Iteration
	)
Begin
{	Test-CLIConnectionB
}	
...
.PARAMETER Usr_cpg 
	Specifies the local user CPG and target user CPG that will be used for volumes 
	that are auto-created. The local CPG will only be used after failover and recover.
.PARAMETER Snp_cpg 
...
param(	[Parameter(Mandatory)]
		...
		[String]	$Usr_cpg,
		[String]	$Snp_cpg,
		[Switch]	$Usr_cpg_unset,
		[Switch]	$Snp_cpg_unset,
...
param(	[Switch]	$Force,
		...
		[Switch]	$Snp_cpg_unset,
		[Parameter(Mandatory)]
		[ValidateSet('auto_failover','no_auto_failover','auto_recover','no_auto_recover','over_per_alert','no_over_per_alert','path_management','no_path_management')]
		[String]	$policy,
		[Parameter(Mandatory)]
		[String]	$GroupName
	)	
...
#>
[CmdletBinding()]
param(	[Parameter(ParameterSetName='e',Mandatory)]	[switch]	$Enable ,
		[Parameter(ParameterSetName='d',Mandatory)]	[switch]	$Disable ,
		[Parameter(Mandatory)]						[String]	$TargetName
	)			
Begin
{	Test-CLIConnectionB
...
param(
		...
		[switch]	$Mirror_Config,
		
		[Parameter(ParameterSetName='NoMirror', Mandatory)]
		[switch]	$No_Mirror_Config,
...
{	$cmd= "setrcopytarget pol "
	...
	if($No_Mirror_Config)		{	$cmd+=" no_mirror_config "	}
	$cmd+="$Target "
	$Result = Invoke-CLICommand -Connection $SANConnection -cmds  $cmd	
	if([string]::IsNullOrEmpty($Result))
...
.PARAMETER TimeInSeconds
	...
	If not specified this defaults to 172800 seconds (48 hours).
.PARAMETER FCIP
