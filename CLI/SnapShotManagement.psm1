...
.PARAMETER exp 
	Specifies the relative time from the current time that volume will expire. <time>[d|D|h|H] <time> is a positive integer value and in the 
	range of 1 - 43,800 hours (1,825 days). Time can be optionally specified in days or hours providing either d or D for day and h or H for 
...
.PARAMETER retain
	...
	<time> is a positive integer value and in the range of 1 - 43,800 hours (1,825 days). Time can be optionally specified 
	in days or hours providing either d or D for day and h or H for hours following the entered time value.
.PARAMETER Match
...
.PARAMETER P
	Starts a copy operation from the specified parent volume (as indicated using the <parent_VV> specifier) to its 
	destination volume (as indicated using the <destination_VV> specifier).
.PARAMETER  R
	Resynchronizes the set of destination volumes (as indicated using the <destination_VV> specifier) with their 
	respective parents using saved snapshots so that only the changes made since the last copy or resynchronization are copied. 
.PARAMETER Halt
	Cancels an ongoing physical copy. 
.PARAMETER S
	Saves snapshots of the parent volume (as indicated with the <parent_VV> specifier) for quick resynchronization and to 
	retain the parent-copy relationships between each parent and destination volume. 
.PARAMETER B
	Use this specifier to block until all the copies are complete. Without this option, the command completes before the copy 
	operations are completed (use the showvv command to check the status of the copy operations).
...
#>
[CmdletBinding()]
param(	[String]	$parent_VV,
		[String]	$destination_VV,
		[String]	$destination_cpg,
		[String]	$VV_name,
		[String]	$wwn,
		[Switch]	$P,
		[Switch]	$R,
...
{	$groupvvcopycmd = "creategroupvvcopy "		
	...
	if($destination_cpg){	$groupvvcopycmd += "$destination_cpg"
							$groupvvcopycmd += ":"
						}
...
.PARAMETER exp 
    Specifies the relative time from the current time that volume will expire.-exp <time>[d|D|h|H] <time> is a positive integer 
	value and in the range of 1 - 43,800 hours (1,825 days). Time can be optionally specified in days or hours providing 
	either d or D for day and h or H for hours following the entered time value. 
.PARAMETER retain
	Specifies the amount of time, relative to the current time, that the volume will be retained. <time> is a positive integer value
	and in the range of 1 - 43,800 hours (1,825 days). Time can be optionally specified in days or hours providing either d or D for 
...
{	if ($svName)
	...
	{	return	"No svName specified for new Snapshot volume. Skip creating Snapshot volume"
	}
}
}
...
.PARAMETER Blocks
	Specifies that this command blocks until the operation is completed. If not specified, the createvvcopy command 
	operation is started as a background task.
.PARAMETER priority
...
{	$PromoteCmd = "promotegroupsv " 	
	...
	else			{	return "No VVNames specified to promote "
					}
	if ($TargetVV)	{	$PromoteCmd += ":"
						$PromoteCmd += "$TargetVV "
					}			
	$result = Invoke-CLICommand -Connection $SANConnection -cmds  $PromoteCmd
	write-verbose "Promoting Group Snapshot with $VVNames with the command --> $PromoteCmd" 
...
.DESCRIPTION
	This command copies the differences of a snapshot back to its base volume, allowing you to revert the base volume to an earlier point in time.
.EXAMPLE
	Push-SnapVolume -name vv1 copies the differences of a snapshot back to its base volume "vv1"
.EXAMPLE
	Push-SnapVolume -target vv23 -name vv1 copies the differences of a snapshot back to target volume "vv23" of volume "vv1"
.PARAMETER name 
...
.PARAMETER PRI
	Specifies the priority of the copy operation when it is started. This option allows the user to control the overall speed of a particular
	task.  If this option is not specified, the promotesv operation is started with default priority of medium. High priority indicates that
	the operation will complete faster. Low priority indicates that the operation will run slower than the default priority task. This option
	cannot be used with -halt option.    
.PARAMETER Online
...
{	$promoCmd = "promotesv"	
	if($target)
		{	if ( !( Test-CLIObject -objectType 'vv' -objectName $target -SANConnection $SANConnection))
				{	Throw "FAILURE : No vv $target found"
				}
	...
	if ( !( Test-CLIObject -objectType 'vv' -objectName $name -SANConnection $SANConnection))
				{	Throw "FAILURE : VV $vvName does not exist. Please use New-Vv to create a VV before creating SV"
				}								
	$promoCmd += " $name "
...
.DESCRIPTION
	Promotes a physical copy back to a regular base volume
.EXAMPLE
    PS::> Push-VvCopy –physicalCopyName volume1 
...
#>
[CmdletBinding()]
param(	[Parameter(Mandatory)]	[String]	$physicalCopyName
	)		
begin
{	Test-CLIConnectionB
}
...
{	if(!( Test-CLIObject -objectType "vv"  -objectName $physicalCopyName -SANConnection $SANConnection))
		{	throw "FAILURE : vv $physicalCopyName does not exist. Please use New-Vv to create a new vv"
		}
...
.DESCRIPTION
	Updates a snapshot Virtual Volume (VV) with a new snapshot.
.EXAMPLE
    PS:> Set-Vv -Name volume1 -Force
	
	snapshot update of snapshot VV "volume1"
.EXAMPLE
    PS:> Set-Vv -Name volume1,volume2 -Force
	
	snapshot update of snapshot VV's "volume1" and "volume2"
.EXAMPLE
    PS:> Set-Vv -Name set:vvset1 -Force
	
	snapshot update of snapshot VVSet "vvset1"
.EXAMPLE
    PS:> Set-Vv -Name set:vvset1,set:vvset2 -Force
	
	snapshot update of snapshot VVSet's "vvset1" and "vvset2"
.EXAMPLE	
	PS:> Set-Vv -Name as2 -RO
.EXAMPLE	
	PS:> Set-Vv -Name as2 -Force -RemoveAndRecreate 
.PARAMETER Name 
	...
	foreach ($i in 0..$limit)
		{	if ( $vvtempnames[$i] -match "^set:")	
			{	$objName = $vvtempnames[$i].Split(':')[1]
				...
				#$objMsg  = $objType
				if(!( Test-CLIObject -objectType $objType  -objectName $vvsetName -SANConnection $SANConnection))
				{	return "FAILURE : No vvset $vvsetName found, vvset $vvsetName does not exist. Please use New-VvSet to create a new vvset "
				}
			}				
			else
			{	$subcmd = $vvtempnames[$i]
				if(!( Test-CLIObject -objectType "vv"  -objectName $subcmd -SANConnection $SANConnection))
				{	return "FAILURE : vv $vvtempnames[$i] does not exist. Please use New-Vv to create a new vv, No vv $subcmd found"
				}
			}
		}		
