...
.DESCRIPTION
	...
	started automatically to redistribute existing volumes to use the new capacity. This facility can be disabled using either the -notune option or 
	setting the AutoAdmitTune system parameter to "no". On systems with more than two nodes, tunesys must always be run manually after disk installation.
.EXAMPLE
.PARAMETER Checkonly
...
{	$plinkresult = Test-PARCli -SANConnection $SANConnection
	if($plinkresult -match "FAILURE :")	{	Return $plinkresult	}
	$Cmd = " admithw "
	...
	Write-verbose "Executing function : Add-Hardware command -->" 
	Return $Result
}
}
...
.PARAMETER Status
	Print status of the current Update-Cage operation in progress or the last executed Update-Cage operation. 
	If any cagenames are specified, result is filtered to only display those cages.
#>
[CmdletBinding()]
param(	[switch]	$A,
		[switch]	$Parallel,
...
{	$Cmd = " shutdownnode "
	...
	else			{	Return "Select at least one from [ Halt | Reboot | Check | Restart]"	}
...
.PARAMETER Port
	Specifies that the operation is performed on port A, port B, or both A and B. If not specified, 
	the operation is performed on both ports A and B.
.PARAMETER F
...
.PARAMETER Remove
	Removes the indicated drive cage (indicated with the <cagename> specifier) from the system. 
	This subcommand fails when the cage has active ports or is in use.
.PARAMETER Pcm
	For DCS11 and DCS12, this specifies that the Power Cooling Module (PCM) will be serviced. 
	For DCN5, this specifies the Power Cooling Battery Module (PCBM) will be serviced.
.PARAMETER Iom
...
{	$Cmd = " servicecage "
	...
	if($CageName)	{	$Cmd += " $CageName " }
	else			{	Return "Cage Name is Mandatory..."	}
	$Result = Invoke-CLICommand -Connection $SANConnection -cmds  $Cmd
...
.DESCRIPTION
	The Set-ServiceNodes command informs the system that a certain component will be replaced, and will cause the system to 
	indicate the physical location of that component.
.EXAMPLE
	Set-ServiceNodes -Start -Nodeid 0
.EXAMPLE
	Set-ServiceNodes -Start -Pci 3 -Nodeid 0
.PARAMETER Start
	Specifies the start of service on a node. If shutting down the node is required to start the service, 
	the command will prompt for confirmation before proceeding further.
.PARAMETER Status
...
.PARAMETER Pci
	Only the service LED corresponding to the PCI card in the specified slot will be 
	illuminated. Accepted values for <slot> are 3 through 5 for HPE 3PAR 600 series systems.
.PARAMETER Fan
	Specifies which node fan will be placed into servicing-mode. For HPE 3PAR 600 series systems, this option is not 
	supported, use servicecage for servicing the Power Cooling Battery Module (PCBM).
.PARAMETER Bat
	Specifies that the node's battery backup unit will be placed into servicing-mode. For HPE 3PAR 600 series systems, 
...
param(	[Parameter(ParameterSetName='Halt', Mandatory)]		[switch]	$Halt,
		...
		[Parameter(ParameterSetName='Reset', Mandatory)]	[switch]	$Restart
	)
Begin
{	Test-CLIConnectionB
}
...
.PARAMETER All
	Specifies that all physical disks with valid IDs and whose firmware is not current are upgraded. If this option is 
	not specified, then either the -w option or PD_ID specifier must be issued on the command line.
.PARAMETER WWN
	Specifies that the firmware of either one or more physical disks, identified by their WWNs, is upgraded. If this 
	option is not specified, then either the -a option or PD_ID specifier must be issued on the command line.
.PARAMETER DiskId
	Specifies that the firmware of either one or more physical disks identified by their IDs (PD_ID) is upgraded. 
	If this specifier is not used, then the -a or -w option must be issued on the command line.
#>
[CmdletBinding()]
param(	[switch]	$Force,
		[switch]	$Skiptest,
		...
		[Parameter(ParameterSetName='ByWWN', Mandatory)]
		[String]	$WWN,
...
.DESCRIPTION
	The Search-ServiceNode command informs the system that a certain component will be replaced, and will 
...
.PARAMETER Ps
	...
	systems, this option is not supported, use servicecage for servicing the Power Cooling Battery Module (PCBM).
.PARAMETER Pci
	Only the service LED corresponding to the PCI card in the specified slot will be illuminated. Accepted values for <slot> 
	are 3 through 5 for HPE 3PAR 600 series systems.
.PARAMETER Fan
...
param(	[Parameter(ParameterSetName='Start',Mandatory)]	[switch]	$Start,
		[Parameter(ParameterSetName='Start',Mandatory)]	[switch]	$Status,
		[Parameter(ParameterSetName='Start',Mandatory)]	[switch]	$End,
		[String]	$Ps,
		[String]	$Pci,
		[String]	$Fan,
		[switch]	$Bat,
...
.PARAMETER F
	...
	before proceeding with its operation.Valid for fips and ssh-keys
.EXAMPLE
...
.EXAMPLE
    ...
    Warning: Disabling FIPS mode requires restarting all system management interfaces, which will terminate ALL 
	existing connections including this one. When that happens, you must reconnect to continue. 
...
.EXAMPLE
    ...
    Warning: Will restart all services that are not enabled, which may terminate ALL existing connections including this one.
    When that happens, you must reconnect to continue. 
...
.EXAMPLE
    ...
    Warning: This action will restart the ssh service, which may terminate ALL existing connections including this one.
    When that happens, you must reconnect to continue.
...
#>
[CmdletBinding()]
param( 	[Parameter(ParameterSetName='FipsEnable',Mandatory)]	[switch]	$FipsEnable,
		[Parameter(ParameterSetName='FipsDisable',Mandatory)]	[switch]	$FipsDisable,
		[Parameter(ParameterSetName='FipsRestart',Mandatory)]	[switch] 	$FipsRestart,
		[Parameter(ParameterSetName='SSHKeyGen',Mandatory)]		[switch]	$SSHKeysGenerate,
		[Parameter(ParameterSetName='SSHKeySync',Mandatory)]	[switch]	$SSHKeysSync,
		[switch]	$F
    )
Begin
{	Test-CLIConnectionB
