...
{	$ListofvLUNs = @()
	...
	if($Result -match "Invalid vv name:")	{	throw "FAILURE : No vv $vvName found"	}
	$Result = $Result | where-object { ($_ -notlike '*total*') -and ($_ -notlike '*------*')} 
...
.PARAMETER Nodelist
	Requests that only VLUNs for specific nodes are displayed. The node list is specified as a series of integers separated 
	by commas (for example 0,1,2). The list can also consist of a single integer (for example 1).
.PARAMETER Slotlist
	Requests that only VLUNs for specific slots are displayed. The slot list is specified as a series of integers separated 
	by commas (for example 0,1,2). The list can also consist of a single integer (for example 1).
.PARAMETER Portlist
	Requests that only VLUNs for specific ports are displayed. The port list is specified as a series of integers separated by 
	commas ((for example 1,2). The list can also consist of a single integer (for example 1).
.PARAMETER Domain_name  
	Shows only the VLUNs whose virtual volumes are in domains with names that match one or more of the <domainname_or_pattern> 
	options. This option does not allow listing objects within a domain of which the user is not a member. Multiple domain names 
	or patterns can be repeated using a comma-separated list.
#>
[CmdletBinding()]
param(	[switch]	$Listcols,
		[String]	$Showcols, 
		[switch]	$ShowsWWN,
		[switch]	$ShowsPathSummary,
		[switch]	$Hostsum,
		[switch]	$ShowsActiveVLUNs,
		[switch]	$ShowsVLUNTemplates,
		[String]	$Hostname,
		[String]	$VV_name,
		[String]	$LUN,
		[String]	$Nodelist,
...
<#
.SYNOPSIS
    The New-vLun command creates a VLUN template that enables export of a Virtual Volume as a SCSI VLUN to a host or 
	hosts. A SCSI VLUN is created when the current system state matches the rule established by the VLUN template
.DESCRIPTION
	The New-vLun command creates a VLUN template that enables export of a Virtual Volume as a SCSI VLUN to a host 
	or hosts. A SCSI VLUN is created when the current system state matches the rule established by the VLUN template.
    ...
    There are four types of VLUN templates:
        ...
        Matched set - created when both hostname and node:slot:port are specified. The VLUN is visible 
		to initiators with the host's WWNs only on the specified port.
...
.PARAMETER vvSet 
	...
	The volume name is provided in the syntax of basename.int.  The VV set name must start with "set:".
.PARAMETER LUN
	Specifies the LUN as an integer from 0 through 16383. Alternatively n+ can be used to indicate a LUN should be auto 
	assigned, but be a minimum of n, or m-n to indicate that a LUN should be chosen in the range m to n. In addition 
	the keyword auto may be used and is treates as 0+.
.PARAMETER HostName
...
.PARAMETER Cnt
	Specifies that a sequence of VLUNs, as specified by the num argument, are exported to the same system port and host 
	that is created. The num argument can be specified as any integer. For each VLUN created, the .int suffix of the 
	VV_name specifier and LUN are incremented by one.
.PARAMETER NoVcn
	Specifies that a VLUN Change Notification (VCN) not be issued after export. For direct connect or loop configurations, 
	a VCN consists of a Fibre Channel Loop Initialization Primitive (LIP). For fabric configurations, a VCN consists of a 
	Registered State Change Notification (RSCN) that is sent to the fabric controller.
.PARAMETER Ovrd
...
{	$cmdVlun = " createvlun -f"
	...
	if ($vvName -Or $vvSet)	
		...
		{	return "No values specified for the parameters vvname. so simply exiting "
		}
	if($LUN)	{	$cmdVlun += " $LUN "	}
	...
	if($Result1 -match "no active paths")		{	return $Result1	}
	elseif([string]::IsNullOrEmpty($Result1))	{	return "Success : $vvName exported to host $objName`n"	}
	else										{	return "FAILURE : While exporting vv $vvName to host $objName Error : $Result1`n"	}		
	return
}	
}
...
.EXAMPLE
	Remove-vLun -vvName vol2 -force -Novc
.EXAMPLE
...
Process
{	if (!(($vvName) -or ($PresentTo)))	{	Throw "Action required: no vv or no host mentioned - simply exiting "	}
	if(!(($force) -or ($whatif)))		{	Throw "FAILURE : no -force or -whatif option selected to remove/dry run of VLUN"	}
	if($PresentTo)						{	$ListofvLuns = Get-vLun -vvName $vvName -PresentTo $PresentTo -SANConnection $SANConnection	}
	else								{	$ListofvLuns = Get-vLun -vvName $vvName -SANConnection $SANConnection	}
	...
	if ($ListofvLuns)
		...
		{	Throw "FAILURE : no vLUN found for $vvName presented to host $PresentTo"
		}
}
}
