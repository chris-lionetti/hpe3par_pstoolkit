...
.DESCRIPTION
	The Get-Alert command displays the status of system alerts. When issues without options, all new customer alerts are displayed.
.EXAMPLE
	Get-Alert -N
.EXAMPLE
	Get-Alert -F
...
.PARAMETER All
	Specifies that all customer alerts are displayed. The format of the alert display is controlled by the following options:
.PARAMETER D
	Specifies that detailed information is displayed. Cannot be specified with the -oneline option.
.PARAMETER Oneline
	Specifies that summary information is displayed in a tabular form with one line per alert. For customer alerts, 
...
.PARAMETER Endt
	...
	The <pattern> argument in the following options is a regular expression pattern that is used to match against the events each option produces.
	...
	showeventlog -type Disk.* -type <tpdtcl client> -sev Major The "-sev Major" displays all events of severity Major and with a type that matches either
	the regular expression Disk.* or <tpdtcl client>.
.PARAMETER Sev
	Specifies that only events with severities that match the specified	pattern(s) are displayed. The supported severities include Fatal,
...
[CmdletBinding()]
param(
	[Parameter(ParameterSetName='NewAll', Mandatory)]
	[Parameter(ParameterSetName='NewId',  Mandatory)]
	[switch]	$New,

	[Parameter(ParameterSetName='AckAll', Mandatory)]
	[Parameter(ParameterSetName='AckId',  Mandatory)]
	[switch]	$Ack,

	[Parameter(ParameterSetName='FixedAll', Mandatory)]
	[Parameter(ParameterSetName='FixedId',  Mandatory)]
	[switch]	$Fixed,
